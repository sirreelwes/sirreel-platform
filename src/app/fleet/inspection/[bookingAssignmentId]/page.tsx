  ...
  const assignment = await prisma.bookingAssignment.findUnique({
    ...
    select: {
      ...
      inspections: {
        where: { type: 'CHECKOUT' },
        select: {
          id: true, inspectionDate: true, inspectorName: true,
      ...
      <div className="max-w-md mx-auto">
        ...
        {existing ? (
          <div className="bg-zinc-800 border border-zinc-700 rounded-xl p-5 text-center">
            ...
            <p className="text-zinc-400 text-sm mt-1">
              {existing.inspectionDate.toISOString().slice(0, 16).replace('T', ' ')} by {existing.inspectorName || existing.inspectedByUser?.name || (existing.inspectedByDriver ? `${existing.inspectedByDriver.firstName} ${existing.inspectedByDriver.lastName}`.trim() + ' (driver, self check-out)' : 'fleet')}
            </p>
