  ...
  return (
    <div>
      <header className="max-w-[1080px] mx-auto px-5 sm:px-8 py-6 flex items-center justify-between gap-4">
        <Wordmark />
        <a href="#request" className="rounded-full bg-[#CC0000] hover:bg-[#A30000] px-5 py-2.5 text-[14px] font-bold text-white no-underline transition-colors">Request a workspace</a>
      </header>
      ...
      <section className="max-w-[1080px] mx-auto px-5 sm:px-8 pt-8 pb-14 sm:pt-16 sm:pb-24 grid lg:grid-cols-[1.15fr_1fr] gap-10 items-center">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <a href="#what" className="rounded-full border-2 border-[#0f2a30] px-6 py-3.5 text-[16px] font-bold text-[#0f2a30] no-underline hover:bg-[#0f2a30] hover:text-white transition-colors">What it does</a>
          </div>
        </div>
        ...
        <div className="rounded-[28px] bg-white border border-[#8FC2CE]/50 p-5 shadow-[0_20px_60px_rgba(15,122,147,0.12)]">
          <div className="flex items-center justify-between text-[12px] font-bold uppercase tracking-[1.6px] text-[#0B5C70]"><span>This week</span><span className="text-[#CC0000]">4 of 6 out</span></div>
            ...
            {[
              ...
              <div key={name as string} className="grid grid-cols-[120px_1fr] items-center gap-3">
                ...
                <div className="relative h-7 rounded-lg bg-[#F1F8F9]">
                  {kind && (
                    <div
                      className={`absolute top-1 bottom-1 rounded-md ${kind === 'out' ? 'bg-[#CC0000]' : kind === 'confirmed' ? 'bg-[#0F7A93]' : 'bg-white border-2 border-dashed border-[#8FC2CE]'}`}
          ...
          <div className="mt-4 flex gap-4 text-[11px] font-semibold text-[#0B5C70]">
            <span className="inline-flex items-center gap-1.5"><i className="w-3 h-3 rounded-sm bg-[#CC0000]" />Out</span>
            <span className="inline-flex items-center gap-1.5"><i className="w-3 h-3 rounded-sm bg-[#0F7A93]" />Confirmed</span>
      ...
      <section id="what" className="bg-white border-y border-[#8FC2CE]/40">
        <div className="max-w-[1080px] mx-auto px-5 sm:px-8 py-16 sm:py-20">
          <h2 className="text-[30px] sm:text-[40px] font-black tracking-[-0.02em]" style={DISPLAY}>What it does</h2>
            ...
            {HQ_PITCH.map((p, i) => {
              ...
              return (
                <div key={p.title} className="rounded-[22px] bg-[#F1F8F9] p-6">
                  <span className="inline-grid place-items-center w-11 h-11 rounded-2xl bg-[#0f2a30] text-white"><Icon className="w-5 h-5" /></span>
      ...
      <section className="max-w-[1080px] mx-auto px-5 sm:px-8 py-16 sm:py-20">
        <h2 className="text-[30px] sm:text-[40px] font-black tracking-[-0.02em]" style={DISPLAY}>Plans</h2>
          ...
          {HQ_PLANS.map((p, i) => (
            <div key={p.key} className={`rounded-[22px] p-7 ${i === 1 ? 'bg-[#0B5C70] text-white' : 'bg-white border border-[#8FC2CE]/50'}`}>
              <div className="flex items-baseline justify-between gap-3">
                <div className="text-[24px] font-black" style={DISPLAY}>{p.name}</div>
                <div className={`text-[13px] font-bold ${i === 1 ? 'text-[#8FC2CE]' : 'text-[#CC0000]'}`}>{p.monthlyUsd == null ? 'Pricing soon' : <><span className="text-[22px]" style={DISPLAY}>${p.monthlyUsd}</span>/mo</>}</div>
              </div>
      ...
      <section id="request" className="bg-white border-t border-[#8FC2CE]/40">
        <div className="max-w-[1080px] mx-auto px-5 sm:px-8 py-16 sm:py-20 grid lg:grid-cols-[1fr_1.2fr] gap-10">
          <div>
            <h2 className="text-[30px] sm:text-[40px] font-black tracking-[-0.02em]" style={DISPLAY}>Request a workspace</h2>
