...
B. Waiver of Subrogation - if the SUBR WVD column shows "Y" on ANY policy row on the ACORD form, this passes. Present on GL only is sufficient.
...
export async function POST(
  ...
  try {
    ...
    // Enforce hard/manageable logic ourselves
    const hardItems = [
    ...
    try {
      ...
      await prisma.$executeRawUnsafe(`ALTER TABLE paperwork_requests ADD COLUMN IF NOT EXISTS coi_admin_approved BOOLEAN DEFAULT FALSE`)
      await prisma.$executeRawUnsafe(`ALTER TABLE paperwork_requests ADD COLUMN IF NOT EXISTS coi_admin_approved_by TEXT`)
      await prisma.$executeRawUnsafe(`ALTER TABLE paperwork_requests ADD COLUMN IF NOT EXISTS coi_admin_approval_note TEXT`)
      await prisma.$executeRawUnsafe(`ALTER TABLE paperwork_requests ADD COLUMN IF NOT EXISTS coi_admin_approved_at TIMESTAMP`)
    } catch {}
