/** POST /api/public/spectiv-drive/[token]/checkout */
import { NextRequest, NextResponse } from 'next/server'
import { checkRateLimit, clientIp } from '@/lib/portal/publicRateLimit'
import { bookingByDriverToken, loadDriverBooking, checkOutBooking } from '@/lib/hq-white-label/driverFlow'

export const dynamic = 'force-dynamic'

export async function POST(req: NextRequest, { params }: { params: { token: string } }) {
  if (!checkRateLimit(`spectiv-drive:${clientIp(req)}`, { windowMs: 60_000, max: 30 }).ok) return NextResponse.json({ error: 'Slow down.' }, { status: 429 })
  ...
  try {
    await checkOutBooking(b.id, body)
    ...
    if (status >= 500) console.error('[spectiv-drive/checkout]', e)
