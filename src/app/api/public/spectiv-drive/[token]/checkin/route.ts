/** POST /api/public/spectiv-drive/[token]/checkin */
import { NextRequest, NextResponse } from 'next/server'
import { checkRateLimit, clientIp } from '@/lib/portal/publicRateLimit'
import { bookingByDriverToken, loadDriverBooking, checkInBooking } from '@/lib/hq-white-label/driverFlow'
...
export async function POST(req: NextRequest, { params }: { params: { token: string } }) {
  ...
  try {
    await checkInBooking(b.id, body)
    ...
    if (status >= 500) console.error('[spectiv-drive/checkin]', e)
