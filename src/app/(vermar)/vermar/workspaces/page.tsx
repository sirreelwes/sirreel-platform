...
function WorkspaceCard({ w, busy, onPatch, onRotate, toast }: {
  ...
  const changeStatus = async (next: Status) => {
    ...
    if (CLOSES.includes(next) && !window.confirm(`Set ${w.vendorName}'s ${HQ_PRODUCT.name} workspace to ${STATUS_LABEL[next]}? Their workspace shows a "closed" screen until it's reopened. Nothing is deleted.`)) return
...
export default function AdminHqWorkspacesPage() {
  ...
  const provision = async (v: VendorWithout) => {
    if (!window.confirm(`Start a 30-day ${HQ_PRODUCT.name} trial for ${v.name}? ${v.email ? `Their contact (${v.email}) is emailed the link.` : 'They have no email on file, so nobody is emailed — copy the link from the card and send it yourself.'}`)) return
    setBusy(v.id)
    ...
    <div>
      <div className="mb-5">
        ...
        <p className="text-sm text-lt-fg2 mt-1 max-w-[72ch]">
          Every partner running their fleet on {HQ_PRODUCT.name}. Who&apos;s on trial, who&apos;s paying, who&apos;s closed, and the link each one logs in
      ...
      {data && (
        <>
          ...
          <p className="text-sm text-lt-fg2 mb-3 max-w-[72ch]">
            ...
            &ldquo;See what {HQ_PRODUCT.name} can do for you&rdquo; does — including emailing their contact the link.
          </p>
