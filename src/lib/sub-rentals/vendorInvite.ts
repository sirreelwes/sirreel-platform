...
export function buildPartnerWelcome(a: {
  ...
  asks.push(`<strong>Send us your vehicle list.</strong> ${a.unitCount === 0 ? 'Reply to this email with every vehicle you want SirReel to be able to book, with a daily and weekly rate for each; we add them and they appear on your page.' : `We have ${a.unitCount} of your vehicles on the page — reply with the rest and their daily and weekly rates, and we add them.`} Once a vehicle is listed you can propose a rate change from the page; it comes to us to accept and never touches a booking already confirmed.`)
  asks.push('<strong>Name your drivers when we book.</strong> Each booking page has “Add a driver” — enter a driver’s email once, they fill in their own profile and license, and from then on you pick from the list. The driver gets their own page with the location and call time.')
