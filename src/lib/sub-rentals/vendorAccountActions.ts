...
/**
 * The partner counter-offers on the DEAL itself.
 *
 ...
 * Same shape as proposeUnitRates — the partner's ask is recorded, HQ is told,
 * and NOTHING about billing moves until someone accepts on the Portals tab.
 * partnerSharePercent stays the deal in the meantime, so a pending proposal
 * cannot quietly change what a quote pays out.
 *
 * The number the partner types is THEIR share, not SirReel's — that is what
 * the page shows them in big type, and asking a partner to counter-offer in
 * terms of the other side's cut is how you get a 20/80 deal by accident.
 * Stored as SirReel's share, converted here, once.
 */
export async function proposeShare(
  vendorId: string,
  input: { partnerPercent: number | null; note: string | null },
): Promise<void> {
  const vendor = await prisma.vendor.findUnique({
    where: { id: vendorId },
    select: { id: true, name: true, partnerSharePercent: true },
  })
  if (!vendor) throw Object.assign(new Error('partner not found'), { status: 404 })

  const p = input.partnerPercent
  if (p == null || !Number.isFinite(p)) throw Object.assign(new Error('Enter the percentage you would keep.'), { status: 400 })
  // 0 and 100 are both real answers ("take it all" / "I keep everything") but
  // outside that there is nothing to negotiate about, only a typo.
  if (p < 0 || p > 100) throw Object.assign(new Error('That has to be between 0 and 100.'), { status: 400 })

  const sirreelShare = Math.round((100 - p) * 100) / 100
  const current = vendor.partnerSharePercent == null ? null : Number(vendor.partnerSharePercent)
  if (current != null && Math.abs(current - sirreelShare) < 0.005) {
    throw Object.assign(new Error('That is the split you already have.'), { status: 400 })
  }

  await prisma.vendor.update({
    where: { id: vendor.id },
    data: {
      proposedSharePercent: sirreelShare,
      shareProposedAt: new Date(),
      shareProposalNote: (input.note ?? '').trim().slice(0, 1000) || null,
    },
  })

  const was = current == null ? 'no split set' : `${Math.round((100 - current) * 100) / 100}% / ${current}%`
  await tellHq(
    `${vendor.name} proposed a different split`,
    `${vendor.name} asked for <strong>${p}% to them / ${sirreelShare}% to SirReel</strong> (currently ${was}).` +
      (input.note ? ` They said: “${(input.note ?? '').trim().slice(0, 1000)}”` : '') +
      ` Nothing has changed — accept or decline it on the Portals tab.`,
    '/crm/portals#vendor',
  )
}

/**
 * Staff: accept moves the proposal into the standing deal; decline drops it.
 *
 * ACCEPTING MAKES THE FILED AGREEMENT STALE. vendorAgreementTerms() prints the
 * split in the Terms box on page one, so a signed agreement now states a deal
 * that is no longer the deal — re-file it (Portals → file the standard
 * agreement) and have them sign again. The caller surfaces that; this returns
 * whether there is a signed agreement to worry about.
 */
export async function resolveShareProposal(
  vendorId: string,
  decision: 'accept' | 'decline',
): Promise<{ sharePercent: number | null; agreementNeedsRefiling: boolean }> {
  const v = await prisma.vendor.findUnique({
    where: { id: vendorId },
    select: { id: true, proposedSharePercent: true, partnerSharePercent: true },
  })
  if (!v) throw Object.assign(new Error('partner not found'), { status: 404 })
  if (v.proposedSharePercent == null) throw Object.assign(new Error('no proposal pending'), { status: 400 })

  const signed = await prisma.vendorAgreement.count({
    where: { vendorId, deletedAt: null, signedAt: { not: null } },
  })

  const updated = await prisma.vendor.update({
    where: { id: vendorId },
    data: {
      ...(decision === 'accept' ? { partnerSharePercent: v.proposedSharePercent } : {}),
      proposedSharePercent: null,
      shareProposedAt: null,
      shareProposalNote: null,
    },
    select: { partnerSharePercent: true },
  })

  return {
    sharePercent: updated.partnerSharePercent == null ? null : Number(updated.partnerSharePercent),
    agreementNeedsRefiling: decision === 'accept' && signed > 0,
  }
}
