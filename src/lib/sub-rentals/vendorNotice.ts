...
// ── "Here is who is collecting it" ───────────────────────────────────────
//
// Wes 2026-09-15. A will-call unit is picked up by the production's own
// person at the partner's counter, and the go note promises we will say who.
// The NAME only: the partner matches it to a licence, and the conduit keeps
// the number, the company and the production to itself as it always has.

export interface VendorCollectorNoticeArgs extends VendorNoticeArgs {
  collectorName: string
  /** A name was already sent and has changed — say so, or it reads as a repeat. */
  changed?: boolean
}

export function buildVendorCollectorNotice(a: VendorCollectorNoticeArgs): { subject: string; html: string; text: string } {
  const range = a.startDate === a.endDate ? fmt(a.startDate) : `${fmt(a.startDate)} — ${fmt(a.endDate)}`
  const subject = `${a.changed ? 'Change of collection' : 'Collecting'} — ${a.vehicleName}, ${range}`
  const lead = a.changed
    ? `The production has changed who is collecting the ${a.vehicleName}.`
    : `Here is who is collecting the ${a.vehicleName}.`
...
<body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color:#f3f4f6;">
    <tr><td align="center" style="padding:24px 12px;">
      <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="max-width:600px;background-color:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.04);">
        ...
        <tr><td style="padding:28px 32px 4px;">
          <p style="font-size:17px;color:${TEXT};margin:0 0 12px;line-height:1.5;">Hi ${escapeHtml(a.vendorName)},</p>
          <p style="font-size:16px;color:${TEXT};margin:0 0 12px;line-height:1.6;">${escapeHtml(lead)} They will check out at your counter with their licence, as any renter would.</p>
        </td></tr>
        <tr><td style="padding:14px 32px 0;">
          <div style="border-left:3px solid ${ACCENT};padding-left:14px;">
            <p style="font-size:12px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:${ACCENT};margin:0 0 2px;">Collecting</p>
            <p style="font-size:22px;font-weight:800;color:${TEXT};margin:0;">${escapeHtml(a.collectorName)}</p>
            <p style="font-size:15px;color:${TEXT};margin:8px 0 0;">${escapeHtml(a.vehicleName)} &middot; ${escapeHtml(range)}</p>
            ${a.reference ? `<p style="font-size:13px;color:${MUTED};margin:2px 0 0;">SirReel reference ${escapeHtml(a.reference)}</p>` : ''}
          </div>
        </td></tr>
        <tr><td align="center" style="padding:26px 32px 4px;">
        ...
        <tr><td style="padding:22px 32px 0;">
          <p style="font-size:13px;color:${MUTED};margin:0;line-height:1.6;">If this changes we will send you the new name. Please reply to this email with any questions rather than contacting the production.</p>
        </td></tr>
        <tr><td style="padding:22px 32px 4px;">
          <p style="font-size:16px;color:${TEXT};margin:0;line-height:1.6;">${escapeHtml(a.agentName)}<br/><span style="color:${MUTED};font-size:14px;">&amp; Team SirReel</span></p>
        </td></tr>
        <tr><td align="center" style="padding:18px 32px 26px;">
          <p style="font-size:12px;color:${MUTED};margin:0;">8500 Lankershim Blvd, Sun Valley CA 91352 &middot; (888) 477-7335</p>
        </td></tr>
  ...
  const text = [
    ...
    '',
    lead,
    `They will check out at your counter with their licence, as any renter would.`,
    '',
    `Collecting: ${a.collectorName}`,
    `${a.vehicleName} · ${range}`,
    ...(a.reference ? [`SirReel reference: ${a.reference}`] : []),
    '',
    `Open the booking page: ${a.vendorUrl}`,
    '',
    `If this changes we will send you the new name. Please reply to this email with any questions rather than contacting the production.`,
    '',
    `— ${a.agentName}`,
    '& Team SirReel',
    '',
