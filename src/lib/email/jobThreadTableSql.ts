 ...
 *   sr_job_thread_alerts — one row per person an URGENT note reached (or
 *                         failed to): channel, outcome. A note is urgent
 *                         when it has alert rows — no column on the note,
 *                         so this table was added with no ALTER.
 *
...
export const JOB_THREAD_TABLES_DDL: AdditiveDdl = {
  tables: ['sr_job_threads', 'sr_job_thread_notes', 'sr_job_thread_alerts'],
  statements: [
    ...
    `CREATE TABLE IF NOT EXISTS "sr_job_thread_alerts" (
       "id" TEXT NOT NULL,
       "job_id" TEXT NOT NULL,
       ...
       "detail" TEXT,
       "created_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
       CONSTRAINT "sr_job_thread_alerts_pkey" PRIMARY KEY ("id")
     )`,
    `CREATE INDEX IF NOT EXISTS "sr_job_thread_alerts_note_id_idx" ON "sr_job_thread_alerts" ("note_id")`,
    `CREATE INDEX IF NOT EXISTS "sr_job_thread_alerts_job_id_created_at_idx" ON "sr_job_thread_alerts" ("job_id", "created_at")`,
  ],
}
