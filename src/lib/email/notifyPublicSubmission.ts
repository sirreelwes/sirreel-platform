...
async function sendClientAck(sub: PublicSubmission): Promise<void> {
  ...
  const html = renderEmailShell({
    ...
    footNote: `Need to change something or add to this? Call us at <a href="${PUBLIC_CONTACT.phoneHref}" style="color:#0F7A93;text-decoration:none;font-weight:600;">${PUBLIC_CONTACT.phone}</a> during business hours. Nothing is confirmed until an agent has been in touch.`,
  })
  ...
  const text = renderEmailText([
    ...
    '',
    `Questions? Call ${PUBLIC_CONTACT.phone} during business hours.`,
