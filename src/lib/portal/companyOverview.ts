...
import {
  findPendingAnnual,
  type PendingAnnual,
} from "@/lib/portal/companyAnnual";

/** Coarse, client-legible job state. */
export type CompanyJobState =
  "UPCOMING" | "ON_JOB" | "WRAPPED" | "HOLD" | "QUOTED";
...
export interface CompanyTermsSummary {
  ...
  /** The account has already asked to be set up on an annual (open ask). */
  annualRequestedAt: Date | null;
...
export function deriveState(
  ...
  if (live.length > 0 && live.every((s) => CLOSED_ORDER_STATUSES.includes(s)))
    return "WRAPPED";
...
export async function buildCompanyTerms(
  companyId: string,
): Promise<CompanyTermsSummary> {
  const now = new Date();
  const [
    company,
    discounts,
  ...
  const filed = agreements
    .filter((a) => !a.pendingSignature)
    .map((a) => {
      const started =
        !a.effectiveDate || a.effectiveDate.getTime() <= now.getTime();
      let notExpired = true;
      if (a.expiryDate) {
  ...
  return {
    negotiatedRates: rates
      .map((r) => ({
        ...
        weeklyRate:
          r.weeklyRate != null && Number(r.weeklyRate) > 0
            ? Number(r.weeklyRate)
            : null,
        weeklyCap: weeklyRateCap(r.inventoryItem.department),
        department: r.inventoryItem.department,
        listDailyRate:
          r.inventoryItem.dailyRate != null &&
          Number(r.inventoryItem.dailyRate) > 0
        ...
        photoPath: (() => {
          ...
          if (!vc) return null;
          const hasImage =
            vc.photos.length > 0 || !!vc.photoUrl || !!r.inventoryItem.imageUrl;
    ...
    // Only surfaced while the ask is still the whole story: an account with
    // coverage, or with a master already waiting to be signed, has moved on
    // from "we'd like one".
    annualRequestedAt:
      coverage || pendingAnnual ? null : (annualRequest?.createdAt ?? null),
...
export async function buildCompanyOverview(
      ...
      select: {
        ...
        orders: {
          select: {
            ...
            signedAgreements: {
              select: { status: true, coveredByAgreementId: true },
            },
          },
        },
      },
    ...
    const signed =
      ...
      job.orders.some((o) =>
        o.signedAgreements.some(
          (a) =>
            (SIGNED_AGREEMENT_STATUSES as readonly string[]).includes(
              a.status,
  ...
  for (const job of jobs) {
    for (const order of job.orders) {
      for (const inv of order.invoices) {
        if (!CLIENT_VISIBLE_INVOICE_STATUSES.includes(inv.status)) continue;
        if (inv.paidAt && inv.paidAt.getTime() >= yearStart.getTime())
          invoicedYtd += toNum(inv.total);
      }
    }
  }
  ...
  for (const job of jobs) {
    for (const inv of rwInvoicesOf(job)) {
      ...
      if (
        inv.paid &&
        inv.invoiceDate &&
        inv.invoiceDate.getTime() >= yearStart.getTime()
      ) {
