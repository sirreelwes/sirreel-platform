...
export function InquiriesSection() {
  ...
  return (
    <section className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-3">
      ...
      ) : newInquiries.length === 0 && followUps.length === 0 ? (
        ...
        <>
          {newInquiries.length === 0 ? (
            <div className="text-xs text-zinc-600 text-center py-3">
              No new inquiries — only follow-ups below.
            </div>
          ) : (
                ...
                <div className="mt-2 divide-y divide-zinc-800/50">
                  {followUps.map((s) => {
                    ...
                    return (
                      <div
                        key={s.emailId}
                        className="py-2 flex items-center justify-between gap-3 flex-wrap opacity-70"
                      >
                        <button
                          type="button"
                          onClick={() => openDrawer(s.emailId)}
                          className="min-w-0 flex-1 text-left rounded hover:bg-zinc-800/30 -mx-1 px-1 py-0.5 transition-colors"
                          aria-label={`Preview thread: ${s.subject || 'no subject'}`}
                        >
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          <button
                            onClick={(e) => { e.stopPropagation(); capture(s.emailId); }}
                            disabled={busy}
                            className="px-2 py-0.5 bg-emerald-700 hover:bg-emerald-600 disabled:bg-emerald-900 text-white text-[10px] font-bold rounded"
                          >
                            {busy ? '…' : 'Capture'}
                          </button>
                          <button
                            onClick={(e) => { e.stopPropagation(); dismiss(s.emailId); }}
                            disabled={busy}
                            className="px-2 py-0.5 text-zinc-500 hover:text-zinc-300 disabled:opacity-50 text-[10px] font-semibold"
                          >
                            Dismiss
                          </button>
                        </div>
