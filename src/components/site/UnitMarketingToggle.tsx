...
export function UnitMarketingToggle({ token, unitId, preview, initial }: { token: string; unitId: string; preview: boolean; initial: boolean }) {
  const [allowed, setAllowed] = useState(initial)
  const [busy, setBusy] = useState(false)
  const [err, setErr] = useState<string | null>(null)
  async function flip() {
    const next = !allowed
    if (!next && !window.confirm('Withdraw permission to market this vehicle? SirReel stops offering it to productions right away. Bookings already confirmed are not affected.')) return
  ...
  return (
    <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
      <span style={{ fontSize: 12, color: allowed ? '#2f7d5d' : '#8a6d1f' }}>
        {allowed ? 'SirReel may offer this vehicle to productions.' : 'Withheld — SirReel is not offering this vehicle.'}
      </span>
