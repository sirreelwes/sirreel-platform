  ...
  return (
    <div className="space-y-3">
      ...
      <div className="border border-lt-hairline rounded-lg p-3">
        ...
        <div className="text-xs text-lt-fg2 mt-1">
          {inv ? <>Emailed to <span className="text-lt-fg">{inv.to}</span> on {new Date(inv.at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}.</> : !welcome ? <span className="text-chip-warn-fg">Locked until the introduction goes out{canSendWelcome ? ' — send it above' : ' (Wes sends that)'}.</span> : stage !== 'partner' ? <span className="text-chip-warn-fg">Locked until they reply and are marked as a new partner{canSendWelcome ? ' — above' : ' (Wes does that)'}.</span> : <span className="text-lt-fg3">The introduction carried their page link. This sends it again on its own, with the first-visit checklist (agreement, {words.many} &amp; rates, {words.drivers ? 'drivers, lot address' : 'delivery contacts, yard address'}).</span>}
        </div>
        <div className="mt-2 flex flex-col sm:flex-row gap-2 sm:items-center">
