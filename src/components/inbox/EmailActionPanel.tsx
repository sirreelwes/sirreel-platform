...
import { AlertTriangle, CheckCircle2, Clapperboard, ClipboardList, CreditCard, DollarSign, FileText, Hourglass, Inbox, Link2, Mail, MessageSquare, PenLine, Send, ShieldCheck, Zap } from 'lucide-react'

const STAGE_CONFIG: Record<string, { label: string; color: string; bg: string; icon: ReactNode }> = {
  new_inquiry:        { label: 'New Inquiry',         color: 'text-cadence-booked-fg',   bg: 'bg-cadence-booked-bg',   icon: <Inbox size={14} aria-hidden /> },
  quote_sent:         { label: 'Quote Sent',           color: 'text-cadence-returned-fg', bg: 'bg-cadence-returned-bg', icon: <ClipboardList size={14} aria-hidden /> },
  negotiating:        { label: 'Negotiating',          color: 'text-chip-warn-fg',  bg: 'bg-chip-warn-bg',  icon: <MessageSquare size={14} aria-hidden /> },
  confirmed:          { label: 'Quote Confirmed',    color: 'text-chip-good-fg',bg: 'bg-chip-good-bg',icon: <CheckCircle2 size={14} aria-hidden /> },
  paperwork_sent:     { label: 'Paperwork Sent',       color: 'text-cadence-booked-fg',   bg: 'bg-cadence-booked-bg',   icon: <Send size={14} aria-hidden /> },
  paperwork_partial:  { label: 'Paperwork Partial',    color: 'text-chip-warn-fg',  bg: 'bg-chip-warn-bg',  icon: <Hourglass size={14} aria-hidden /> },
  paperwork_complete: { label: 'Paperwork Complete', color: 'text-chip-good-fg',bg: 'bg-chip-good-bg',icon: <CheckCircle2 size={14} aria-hidden /> },
  active_job:         { label: 'Active Job',           color: 'text-chip-good-fg',bg: 'bg-chip-good-bg',icon: <Clapperboard size={14} aria-hidden /> },
  damage_claim:       { label: 'Damage/Claim',         color: 'text-chip-bad-fg',    bg: 'bg-chip-bad-bg',    icon: <AlertTriangle size={14} aria-hidden /> },
  billing:            { label: 'Billing',              color: 'text-chip-warn-fg',  bg: 'bg-chip-warn-bg',  icon: <CreditCard size={14} aria-hidden /> },
  general:            { label: 'General',              color: 'text-chip-neutral-fg',   bg: 'bg-chip-neutral-bg',   icon: <Mail size={14} aria-hidden /> },
};

const ACTION_CONFIG: Record<string, { label: string; color: string; icon: ReactNode }> = {
  create_booking_and_quote: { label: 'Create Booking & Send Quote', color: 'bg-lt-fg hover:bg-black', icon: <ClipboardList size={14} aria-hidden /> },
  send_revised_quote:       { label: 'Send Revised Quote',          color: 'bg-lt-fg hover:bg-black', icon: <DollarSign size={14} aria-hidden /> },
  send_portal_link:         { label: 'Send Portal Link',            color: 'bg-cadence-on-rental-bar hover:opacity-90', icon: <Link2 size={14} aria-hidden /> },
  follow_up_wc:             { label: 'Follow Up — Workers Comp',    color: 'bg-cadence-returning-today-bar hover:opacity-90', icon: <FileText size={14} aria-hidden /> },
  follow_up_coi:            { label: 'Follow Up — COI',             color: 'bg-cadence-returning-today-bar hover:opacity-90', icon: <ShieldCheck size={14} aria-hidden /> },
  follow_up_paperwork:      { label: 'Follow Up — Paperwork',       color: 'bg-cadence-returning-today-bar hover:opacity-90', icon: <PenLine size={14} aria-hidden /> },
  confirm_card_on_file:     { label: 'Confirm Card on File',        color: 'bg-lt-fg hover:bg-black', icon: <CreditCard size={14} aria-hidden /> },
  loop_in_claims:           { label: 'Loop in Claims Team',         color: 'bg-chip-bad-fg hover:opacity-90', icon: <AlertTriangle size={14} aria-hidden /> },
  mark_complete:            { label: 'Mark Paperwork Complete',     color: 'bg-cadence-on-rental-bar hover:opacity-90', icon: <CheckCircle2 size={14} aria-hidden /> },
};
    ...
    <div className="space-y-3">
      ...
      {!detection ? (
        <button onClick={detect} disabled={detecting}
          ...
          {detecting ? <><span className="animate-spin"><Zap size={16} aria-hidden /></span> Analyzing email...</> : 'Analyze with AI'}
        </button>
      ) : (
        <div className="space-y-3">
          {/* Stage badge */}
