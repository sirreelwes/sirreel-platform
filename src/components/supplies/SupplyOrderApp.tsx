...
export function SupplyOrderApp({ submitEndpoint, signInHref = '/portal/auth/sign-in', focusMode = false }: SupplyOrderAppProps) {
  ...
  useEffect(() => {
    ...
    if (!target) return
    // Two things had to change for this to actually land — measured on the
    // production build, where `?category=` had quietly stopped scrolling
    // long before `?q=` existed:
    //
    // 1. INSTANT, not 'smooth'. The grid is still painting (thumbnails,
    //    sticky bar) when the animation starts and Chrome cancels it.
    // 2. RE-ASSERT after paint. A scroll issued during hydration is undone
    //    when the document finishes loading — the page snaps back to 0. So
    //    fire again on a later frame, and only while the visitor is still
    //    at the top (if they've already scrolled, that's their scroll and
    //    we leave it alone).
    const jump = () => target.scrollIntoView({ behavior: 'auto', block: 'start' })
    requestAnimationFrame(jump)
    const settle = setTimeout(() => {
      if (window.scrollY < 8) jump()
    }, 250)
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f4f1ea] text-[#0c0c0d]" style={{ fontFamily: '"Hanken Grotesk", Inter, system-ui, sans-serif' }}>
      {/* ── STREAMLINED HEADER (mobile focus mode only) ───────────
          Slim black bar in a 3-column grid so the logo sits in its OWN
          centered, in-flow cell (a reliable tap target — the old absolute
          center overlapped its siblings and swallowed taps). Left: a
          "← Home" back button + call-us phone icon. Center: S-mark logo
          (→ /home). Right: Login (magic-link flow). Cart persists on
          return via sessionStorage. Shown ONLY on mobile from-home. */}
      {focusMode && (
          ...
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 px-4 h-14">
            ...
            <div className="flex items-center gap-1.5 justify-self-start">
              ...
              <a
                ...
                className="w-9 h-9 inline-flex items-center justify-center text-[#cfc9bd] hover:text-white"
              >
      ...
      <div className="max-w-[1480px] mx-auto px-5 relative">
        <div className="grid gap-8 pt-7 pb-[120px] lg:grid-cols-[1fr_374px] items-start">
          ...
          <main className="min-w-0">
            ...
            {/* catalog grid. flex-col + `order-*`: Reserve Vehicles renders
                BELOW the Production Supplies items at ALL widths (Wes, 2026-07
                — previously desktop kept vehicles on top). The only
                vehicles-primary surface is the /vehicles catalog (Trucks &
                Vans tile routes there), which is a separate page — vehicles
                stay the product there. Same blocks, same components — CSS
                order only. Error/empty states keep order-0 (stay on top). */}
            <div className="mt-3 flex flex-col">
              ...
              {!debouncedQuery && data && (
                <div ref={supplyGridRef} className={`order-1 mt-9 scroll-mt-[120px] ${focusHideMobile}`}>
                  <div className="flex items-baseline gap-3.5 mb-3.5">
                    <h2 className="font-extrabold tracking-tight text-[23px] text-[#0c0c0d]" style={{ fontFamily: 'Archivo, sans-serif' }}>
                      Production Supplies
                    </h2>
              ...
              {displaySections.map((cat) => {
                ...
                return (
                <section key={cat.label} className="order-1 mt-8 scroll-mt-[200px]">
                  <div className="flex items-baseline gap-3.5 mb-3.5">
                    <div>
                      {heroTitle && (
