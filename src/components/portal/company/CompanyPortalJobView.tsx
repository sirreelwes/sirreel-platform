/**
 * One show's paperwork inside the account portal — the body shared by the
 * client page (/portal/company/[companyId]/job/[jobId]) and HQ's preview
 * (/crm/portals/preview/company/[companyId]/job/[jobId]).
 *
 ...
 * the point.
 *
 * In preview the invoice PDFs open through the staff routes (the portal's
 * own 404 without a client session); the per-order signed copy has no
 * staff twin here and is shown inert.
 */

import Link from 'next/link'
import { ArrowLeft, BadgeCheck, Download, FileText, Mail, Phone } from 'lucide-react'
import type { CompanyJobDetail } from '@/lib/portal/companyJobDetail'
import { PORTAL } from '@/lib/brand/portalTokens'
import { companyPortalLinks } from '@/components/portal/company/CompanyPortalView'
...
/** The staff-gated twin of a portal invoice PDF link. */
function staffInvoiceHref(companyId: string, inv: { id: string; pdfHref?: string }): string {
  const rw = inv.pdfHref?.match(/\/rw-invoice\/([^/]+)\/pdf/)
  if (rw) return `/api/rentalworks/invoices/${rw[1]}/pdf`
              ...
              {job.agreements.map((a) => (
                <div key={a.id} className="p-4 flex items-center justify-between gap-4 flex-wrap">
                  ...
                  {a.hasPdf &&
                    (preview ? (
                      <span
                        title="Open the signed copy from the job page in HQ"
                        className="inline-flex items-center gap-1 text-xs font-semibold text-zinc-400 border border-zinc-200 rounded-lg px-2.5 py-1.5 shrink-0 cursor-not-allowed"
                      >
