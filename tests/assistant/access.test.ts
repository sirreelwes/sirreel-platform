...
check('partner only → partner', resolveLevel({ isPartner: true }) === 'partner')
check('contact beats partner', resolveLevel({ isContact: true, isPartner: true }) === 'contact')
check('HQ role beats partner', resolveLevel({ userRole: 'AGENT', isPartner: true }) === 'staff')
check('BLOCKED beats partner', resolveLevel({ grantLevel: 'blocked', isPartner: true }) === 'blocked')
...
check('partner never opens a contact or staff gate', !atLeast('partner', 'contact') && !atLeast('partner', 'staff') && atLeast('partner', 'public'))
check('partner cannot be granted by hand', levelFromGrant('PARTNER') === 'public')
check('partner legend promises no production names', LEVEL_CAPABILITIES.partner.can.some((c) => /Never the production/.test(c)))
...
check('every level has a legend entry', (['blocked', 'public', 'partner', 'contact', 'staff', 'admin'] as const).every((l) => LEVEL_CAPABILITIES[l].can.length > 0))
